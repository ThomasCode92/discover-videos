...
export async function findWatchedVideosByUserId(token, userId) {
  const operationDoc = /* GraphQL */ `
    query FindWatchedVideosByUserId($userId: String!) {
      stats(where: { user_id: { _eq: $userId }, watched: { _eq: true } }) {
        video_id
      }
    }
  `;
...
export async function findFavouredVideosByUserId(token, userId) {
  const operationDoc = /* GraphQL */ `
    query FindFavouredVideosByUserId($userId: String!) {
      stats(where: { user_id: { _eq: $userId }, favoured: { _eq: true } }) {
        video_id
      }
    }
  `;
